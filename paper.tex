...
Yet, empirical studies testing the theoretical implications and estabilishing the stylized facts on expecations and more fundamentally 
on how economic agents take and use information are still limited, primarily because of the limited availablity of data to use.
In this paper, we study a relatively underused dataset on market analysts forecast of earnings of firms called IBES (Institutional Brokers' Estimayte System).
to show both cross-sectional and time-series patterns in analyst forecasts.
Focusinng on Japanese publicly traded firms, we use data taken from IBES and merge with NikkeiNeeds and DBJ data to construct time-series indices 
...
This paper examines the properties and determinants of analyst forecast dispersion and forecast errors, 
using a comprehensive dataset of Japanese firms from 1985 to 2023. 
We construct novel time-series indices of forecast dispersion and errors, 
which provide a real-time measure of firm-level uncertainty. 
We then explore the relationships between these indices and various firm characteristics, 
as well as macroeconomic and financial market indicators.
...
We interpret this as evidence of information spillovers and learning among analysts.

Third, the forecast dispersion and error indices exhibit significant time-series variation, 
with spikes corresponding to major economic and geopolitical events. 
...
rising during downturns and periods of stock market weakness.
...
The indices we develop can serve as a valuable tool for policymakers and market participants
 to monitor uncertainty in real time. 

Our paper contributes to several strands of literature.

Firstly, we contribute to the empirical literature on uncertainty. Several proxies have been de-
veloped within the literature, ranging from the volatility of GDP or stock prices to disagreement
and forecast errors in survey data, as uncertainty is difficult to identify. For example, Leahy and
Whited (1996) construct a measure of uncertainty from the volatility of stock returns for indi-
vidual firms. Guiso and Parigi (1999) use survey data on demand forecasts by Italian firms to
infer the level of uncertainty facing these individual firms.8 Bond et al. (2005) consider several
Guiso and Parigi (1999) use 3-point probability distributions from the Bank of Italy Survey of Investment
...
In this paper, I use data on earnings forecasts by individual analysts as in
Bond et al. (2005); however, I examine not only the average cross-sectional distribution but
also the cyclical changes of this uncertainty measure. Bachmann et al. (2013) use survey data
...
of uncertainty. I also use forecast disagreement to measure uncertainty
...
The measure of forecast dispersion and forecast error appear to refelct the level of imperfect information about a firm's future earnings.
In particular, the measure of forecast dispersion is a type of second momemnt proxy in terms of the level of imperfect informaiton, which can be 
thought of as the level of uncertainty about a firm's future earnings.
While it is a measure of such from the perspective of market analysts, not from the perspective of business managers, 
...
Other spikes seen in the figure include when the Bank of Japan cut the discoun rate to 1.00 percent in 1994, 
during times when there was a banking crises in Japan, on top of the Asian financial crisis around 1998, 
...
examine their basic feactures in capturing times when uncertainy is high and comapare them with the EPU index and the Nikkei 225 index.
...
We argue that these indices are not only relevant for the level of uncertainy srrounding business perfromance in general but also 
